...
class RegisterAPI(generics.GenericAPIView):
    ...
    def post(self, request):
        user = request.data
        if (
            NewUser.objects.filter(email=user["email"])
            .filter(is_verified=False)
            .exists()
        ):
            NewUser.objects.filter(email=user["email"]).delete()
        else:
            if (
                NewUser.objects.filter(email=user["email"])
...
class VerifyEmail(views.APIView):
    ...
    def get(self, request):
        ...
        try:
            payload = jwt.decode(
                token, settings.SECRET_KEY, verify=False, algorithms="HS256"
            )
...
class RequestPasswordResetMail(generics.GenericAPIView):
